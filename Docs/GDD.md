...
- The galaxy uses **hex-style adjacency logic** internally for fair distances and simple pathfinding.
- The visible map can be drawn as a soft rectangular / offset grid.
- Default map size: **500 × 500 hex sectors** (250,000 sectors total).
- Each hex sector represents a **100,000 × 100,000 unit square** in local space for placing stars, planets, ports, and gates.
...
3. **Star System Details**
   - For each sector that is a Star System:
     - Generate **0–7 planets** (simple uniform or weighted random).
     - Assign basic planet types (can be refined later).
     - Store local positions for star and planets within the 100,000-unit hex.
...
5. **Homeworld Selection**
   - For each playable species:
     - Find candidate **Star System** sectors in the **Core or Mid** bands that have at least one planet.
     - Randomly choose one system as that species’ **Homeworld**.
     - Mark one planet in that system as the **homeworld planet**.
     - Place a starting **port**, **base**, and **warp gate** in orbit around that planet.
...
- Each homeworld system starts with:
  - one homeworld planet,
  - one starter port,
  - one starter base,
  - one starter warp gate.
